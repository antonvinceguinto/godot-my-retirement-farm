...
static func get_crop_data(type: CropType) -> CropData:
	...
	match type:
		CropType.TOMATO:
			...
			data.stage_frames = [0, 1, 2, 3, 4, 5]; # 6 frames for 6 stages
		...
		CropType.POTATO:
			...
			data.stage_frames = [0, 1, 2, 3, 3, 3]; # Repeat mature frame for unused stages
		...
		CropType.PUMPKIN:
			...
			data.stage_frames = [0, 1, 2, 3, 4, 4]; # Repeat mature frame for unused stages
